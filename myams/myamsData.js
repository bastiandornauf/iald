var yamsInfo = {
	...
	version: "1.1",
	rules: "<h2>The YAMS rules:</h2>" + "<h3>Setup Sequence:</h3><ul><li>Choose lists.</li><li>Draw 6 and discard 2 mission cards. (you can change these numbers according to your taste)</li><li>Roll off for Initiative v. Deploy.</li><li>Place the Civvie and crate d20&Prime; in from opposite table edges.</li><li>Deploy including flags, no deployment within ZoC of Civvie or crate.</li><li>Double-or-Quits, start first turn.</li></ul>" + "<h3>Overview:</h3><p>At the start of the game, each player draws a number of objective cards and then chooses some of them to discard. During the game and at the end of the game, different objective cards are revealed as their conditions are met. Each card revealed in this way is worth 1VP.</p>" + "<h3>Pick Armies, Select Cards</h3> <p>After choosing their army list each player randomly picks six cards from their pile of objective cards and discards two of their choice leaving them with four objectives, each worth one VP. These cards are kept secret, the other player does not know your objectives!</p> <p>To alter the total number of points available in a game alter the number of cards kept and discarded.</p>" + "<h3>Double-or-Quits, Start Game</h3>" + "<p>Starting with the player who will get first turn, each player can announce ʻdoubleor-quitsʼ. They then reveal one of their objective cards. If the conditions are acheived, this card is worth 2VP instead of 1VP. Ambush cards are then played, starting with the first player.</p>" + "<h3>Deployment</h3>" + "<p>The player that gets deployment should place the Civvie and Crate before any models are deployed. Place the Civvie on the centre line of the table, d20” from the left table edge and the Crate d20” in from the opposite table edge, also on the centre line.</p>" + "<p>When deploying their main force, each player must also deploy a flag/HQ marker in their deployment zone. This is the size and shape of a 25mm base, has ARM0 and BTS0 and is destroyed by any failed ARM or BTS roll. It cannot be moved and counts as having CH: Camouflage against shooting.</p>" + "<p>Deploy reserve models as normal.</p>" + "<p><span class='bold'>Important note:</span> models cannot deploy with the Civvie or Crate within their 8” ZoC.</p>" + "<h3>Notes</h3>" + "<h4>Civvie</h4><ul> <li>The Civvie is neutral.</li> <li>It cannot be shot at unless it is synched to an enemy model.</li> <li>Once synched to an enemy model it counts as an enemy model for AROs, deployable equipment and Impetuous moves.</li> <li>Anyone can attack the Civvie in close combat.</li> <li>Models cannot deploy with the Civvie in their ZoC.</li></ul>" + "<h4>Crate</h4>" + "<ul><li>The crate cannot be moved or destroyed.</li><li>Models cannot deploy with the Crate in their ZoC.</li></ul>" + "<h4>Triangulation</h4><ul><li>Each model carries a single beacon.</li></ul>" + "<h4>Forward Base/Forward Target</h4>" + "<ul><li>The terrain piece must be wholly within 6&Prime; of the centre line between the playerʼs deployment zones.</li></ul>" + "<h4>General</h4>" + "<ul><li>ʻCannot be done in the first turnʼ cards canʼt be used at all in the first game turn.</li></ul>" + "",
	draw: 6,
	keep: 4
};
...
var welcomeStack = [{
	...
	text: "<p>To play a game just click on the DRAW button above. " + yamsInfo.draw + " cards will be randomly drawn. Then you can discard " + (yamsInfo.draw - yamsInfo.keep) + " by clicking the discard buttons. Clicking on a header of a card you can activate a mode where only this card is shown, so you can reveal it to your opponent." + "<p>Press RULES to see the YAMS rules.</p>" + "<p>Press HELP to get a help text for MobileYAMS</p>" + "<p></p>",
	color: red,
	objective: "",
	conditions: "",
	reveal: ""
	...
	text: "MobileYAMS is a port of <a href='http://wargamingtrader.com/yams'>YAMS</a> by <a href='http://wargamingtrader.com/'>wargamingtrader</a>"+
		"for <a href='http://www.infinitythegame.com/infinity/'>infinity the game</a> by <a href='http://www.corvusbelli.com'>Corvus Belli</a>"+
		"and was done by <a href='mailto:bdornauf@gmail.com'>Bastian Dornauf</a>."
}];
